...
public class ShellSetting extends JFrame {
    ...
    private JComboBox<String> chunkComboBox;
    ...
    public ShellSetting(String id, String defaultGroup) {
        ...
        GBC gbcLChunk = (new GBC(0, 13)).setInsets(5, -40, 0, 0);
        GBC gbcChunk = (new GBC(1, 13, 3, 1)).setInsets(5, 20, 0, 0);

        GBC gbcSet = (new GBC(0, 14, 3, 1)).setInsets(5, 20, 0, 0);
        GBC gbcTest = (new GBC(1, 14, 3, 1)).setInsets(5, 20, 0, 0);
        ...
        this.basicsPanel.add(this.isChunkLabel, gbcLChunk);
        this.basicsPanel.add(this.proxyComboBox, gbcProxy);
        this.basicsPanel.add(this.chunkComboBox, gbcChunk);
        ...
        this.addToComboBox(this.chunkComboBox, new String[]{"true", "false"});
    ...
    private void initUpdateShellValue(String id) {
        ...
        this.chunkComboBox.setSelectedItem(this.shellContext.getIsChunk());
...
//                Log.log(String.format("CloseShellState: %s\tShellId: %s\tShellHash: %s", this.shellContext.getPayloadModule().close(), this.shellContext.getId(), this.shellContext.hashCode()), new Object[0]);
                Log.log(String.format("CloseShell======ShellId: %s\tShellHash: %s", this.shellContext.getId(), this.shellContext.hashCode()), new Object[0]);
            } else {
    ...
    private boolean updateTempShellEntity() {
        ...
        String isChunk = (String)this.chunkComboBox.getSelectedItem();
        ...
        if (url != null && url.trim().length() > 0 && password != null && password.trim().length() > 0 && secretKey != null && secretKey.trim().length() > 0 && payload != null && payload.trim().length() > 0 && cryption != null && cryption.trim().length() > 0 && encoding != null && encoding.trim().length() > 0) {
            ...
            this.shellContext.setIsChunk(isChunk == null ? "": isChunk);
            return true;
        } else {
